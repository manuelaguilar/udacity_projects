...
class OthelloLogic:
    ...
    def _make_move(self, CP, move):
        ...
        # this is just to follow the implementation in BASIC
        ...
        for f in range(0,8):
            ...
            if col+self.D[f][1] == 9 or col+self.D[f][1] == 0:
                ...
                if CF == 1:
                    self.C[f] = f
                continue
            print "Debug: posible placements", row, col, self.D[f]
    ...
    def _cpu_move(self):
        if self.game_mode == "TWO PLAYER":
            return False, "I'm not supposed to be playing. You have to be in single"
        ...
        NF=0
        MX=0
        #needed to flag end of f loop from within while block
        EL_1=False
            ...
            for y in range(0,8):
                ...
                for f in range(0,8):
                    ...
                    while True:
                        XP=XP+DY
                        YP=YP+DX
                        if (XP+1==0 or XP+1==9) or (YP+1==0 or YP+1==9): 
                            break
                        if self.B[XP][YP] == 1:
                            RF=1
                        else:
